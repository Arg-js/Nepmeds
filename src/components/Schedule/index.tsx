...
const ScheduleComponent: React.FC<IScheduleComponent> = memo(
  function ScheduleComponent({ selectedFullDate }) {
    const availabilityData = useDoctorAvailability().data;
    const [isSingleAvailabilityLoading, setIsSingleAvailabilityLoading] =
      useState(false);

    const listOfTimeObject = ListOfTimeObject(
      availabilityData,
      selectedFullDate
    );
    ...
    const shouldColorBlock = useCallback(
      (time: string, minute: string) => {
        const timeConcat =
          String(time.split(":")[0]) +
          ":" +
        ...
        return listOfTimeObject.find(item =>
          item?.timeFrame?.includes(timeConcat)
        );
      },
      [listOfTimeObject]
    );

    const shouldAddAppBox = useCallback(
      (time: string, minute: string): undefined | IAvailabilityBookingInfo => {
        const timeConcat =
          String(time.split(":")[0]) +
          ":" +
          minuteTime[minute as keyof typeof minuteTime] +
      ...
      [listOfTimeObject, timeFrameWithBookingInfo]
    );
